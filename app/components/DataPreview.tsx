...
const buildMillerPDF = async (params: {
  ...
  const { companyHeader, billNo, billDate, periodOfBilling, millerName, millerLoc, rows, totalQuantity, totalCommission, bank, companyPhone, companyPAN, companyGST, showQRCode = true, customQRImage, showGstInPdf = true } = params;
...
const DataPreview: React.FC<Props> = ({
  ...
  const [dueListIncludeQRCode, setDueListIncludeQRCode] = useState(showQRCode);
  ...
  const exportToPDF = async () => {
    const doc = await buildMillerPDF({
      ...
      periodOfBilling,
      millerName: selectedMiller && selectedMiller !== 'all' ? selectedMiller : 'All Millers',
      rows: calculatedRows,
      totalQuantity,
      totalCommission,
  ...
  const generateOneMiller = async (miller: string, billNo: number, millerRows: CalcRow[], rawRows?: any[]) => {
    setGeneratingMiller(miller);
    const millerQty = millerRows.reduce((s, r) => s + parseFloat(r.quantity), 0);
    const millerComm = millerRows.reduce((s, r) => s + parseFloat(r.commission), 0);

    let mLoc = selectedShopLoc || '';
    const sourceRows = (rawRows && rawRows.length > 0)
      ? rawRows
      : data.filter(r => (r['MILLER NAME'] || r['MILLER'] || '').toString().trim().toLowerCase() === miller.toLowerCase());
    for (const r of sourceRows) {
      const loc = r['SHOP LOC'] || r['SHOP LOCATION'] || r['PLACE'] || r['LOCATION'] || r['CITY'] || r['ROAD'] || r['MILLER LOCATION'] || '';
      if (loc && String(loc).trim()) {
        mLoc = String(loc).trim();
        break;
      }
    }
  ...
  const handleGenerateAllInPanel = async () => {
    ...
    for (const { miller, billNo, rows } of millerList) {
      ...
      await generateOneMiller(miller, billNo, buildRows(rows, commissionType, commissionRate, fixedRate), rows);
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.5rem' }}>
            {millerList.map(({ miller, billNo, rows }) => {
              ...
              const qty = millerRows.reduce((s, r) => s + parseFloat(r.quantity), 0);
              ...
              return (
                <div key={miller} style={{
                  ...
                  {isDone ? (
                    ...
                    <button
                      onClick={() => generateOneMiller(miller, billNo, millerRows, rows)}
